...
from dwasim.similarity import cache_identity, use_cache, pair_discrepancies, global_normalizers, mixture_affinity
from sklearn.metrics.pairwise import manhattan_distances

from dwasim.paths import CACHE_ROOT, PROCESSED_ROOT, RESULTS_ROOT
from dwasim.data import PATHS, full_profile, multiply_chain
from dwasim.evaluation import deterministic_topk
from dwasim.baselines import BASELINE_CACHE_TAG, hetesim_affinity, index_fingerprint, pathsim_affinity
...
def multilabel_prediction(
    ...
    if target_cardinality is not None:
        count = min(max(int(target_cardinality), 1), reference_labels.shape[1])
        order = np.argsort(-scores, axis=1, kind="stable")[:, :count]
        prediction[:] = 0
        np.put_along_axis(prediction, order, 1, axis=1)
...
def component_cache(path_name: str, split_name: str, query_ids: np.ndarray, reference_ids: np.ndarray) -> Path:
    identity = cache_identity(PROCESSED_ROOT, DATASET, path_name, split_name, query_ids, reference_ids)
    return CACHE_ROOT / f"imdb_pair_components_{split_name}_{path_name}_{identity}.npz"
...
def pair_components(
    ...
    cache_path = component_cache(path_name, split_name, query_ids, reference_ids)
    if use_cache(cache_path):
        with np.load(cache_path) as cached:
            return {key: cached[key] for key in cached.files}
    profiles = full_profile(DATASET, path_name).tocsr().astype(np.float64)
    values = pair_discrepancies(profiles[query_ids], profiles[reference_ids], include_hamming=True)
    identity = cache_identity(PROCESSED_ROOT, DATASET, path_name, "global-normalizers")
    normalizer_path = CACHE_ROOT / f"imdb_normalizers_{path_name}_{identity}.npz"
    if use_cache(normalizer_path):
        with np.load(normalizer_path) as normalizers:
            b0, b1 = float(normalizers["b0"]), float(normalizers["b1"])
    else:
        b0, b1 = global_normalizers(profiles)
        np.savez_compressed(normalizer_path, b0=b0, b1=b1)
    payload = {name: value.astype(np.float32) for name, value in values.items()}
    payload["bray_curtis"] = payload["bray"]
    payload["b0"], payload["b1"] = np.asarray(b0), np.asarray(b1)
...
def component_affinity(
    ...
    if formulation == "relative":
        ...
        b0 = float(components["b0"])
        b1 = float(components["b1"])
        if b0 <= 0 and b1 <= 0:
            return np.ones_like(components["hamming"], dtype=np.float32)
        distance = (
            float(weight) * components["hamming"] / max(b0, np.finfo(float).tiny)
            + (1.0 - float(weight))
            * components["magnitude"]
            / max(b1, np.finfo(float).tiny)
        )
    else:
...
def baseline_affinity(
    ...
    data_key = cache_identity(PROCESSED_ROOT, DATASET, path_name)
    cache_path = CACHE_ROOT / (
        f"imdb_{BASELINE_CACHE_TAG}_{method.lower()}_{path_name}_"
        f"q{query_key}_r{reference_key}_{data_key}.npz"
    )
    if use_cache(cache_path):
        with np.load(cache_path) as cached:
...
def select_three_components(train_components, train_ids, train_labels, folds, one_hot=False):
    grid = [tuple(row) for row in np.eye(3)] if one_hot else simplex_weights(3)
    selections, diagnostics = {}, {}
    for path_name in PATH_NAMES:
        rows = []
        for weights in grid:
            affinity = mixture_affinity(train_components[path_name], weights)
            scores = []
            for reference, validation in folds:
                view = affinity[np.ix_(validation, reference)]
                neighbours = deterministic_topk(view, train_ids[reference], min(10, len(reference)), largest=True)
                prediction = multilabel_prediction(view, neighbours, train_labels[reference], 1., 0., 0.5)
                scores.append(score_prediction(train_labels[validation], prediction))
            rows.append({"weights": list(weights), "macro_f1_mean": float(np.mean([s[0] for s in scores])),
                         "micro_f1_mean": float(np.mean([s[1] for s in scores]))})
        best = max(rows, key=lambda row: (row["macro_f1_mean"], row["micro_f1_mean"],
                                         -np.count_nonzero(row["weights"]), row["weights"][1]))
        selections[path_name] = best["weights"]
        diagnostics[path_name] = rows
...
def fixed_external_prediction(
    ...
    return multilabel_prediction(
        ...
        config["prior_power"],
        config["threshold"],
        target_cardinality=config.get("target_cardinality"),
    )
...
def main() -> None:
    ...
    with PeakRSS() as memory:
        ...
        for method_name, one_hot in (("TriComponentDWASim", False), ("OneComponentSelector", True)):
            chosen, component_grid = select_three_components(train_components, train_ids, train_labels, folds, one_hot)
            train_affinities = [mixture_affinity(train_components[path], chosen[path]) for path in PATH_NAMES]
            external_affinities = [mixture_affinity(external_components[path], chosen[path]) for path in PATH_NAMES]
            selected_pipeline = select_pipeline(train_affinities, train_ids, train_labels, folds)
            config = selected_pipeline["selected"]
            predictions[method_name] = fixed_external_prediction(external_affinities, train_ids, train_labels, config)
            selections[method_name] = {"selected_component_weights": chosen,
                                      "component_validation": component_grid, **selected_pipeline}
            if not one_hot:
                calibrated = dict(config, target_cardinality=int(np.rint(train_labels.sum(axis=1).mean())))
                predictions["CardinalityCalibratedDWASim"] = fixed_external_prediction(
                    external_affinities, train_ids, train_labels, calibrated)
                selections["CardinalityCalibratedDWASim"] = {"selected": calibrated,
                    "note": "Training mean label count, rounded once; no test-label calibration."}

        for method_name, component_weights in (("Cosine", (0., 0., 1.)), ("FixedEqualMixture", (1/3, 1/3, 1/3))):
            train_affinities = [mixture_affinity(train_components[path], component_weights) for path in PATH_NAMES]
            external_affinities = [mixture_affinity(external_components[path], component_weights) for path in PATH_NAMES]
            selected_pipeline = select_pipeline(train_affinities, train_ids, train_labels, folds)
            predictions[method_name] = fixed_external_prediction(external_affinities, train_ids, train_labels,
                                                                 selected_pipeline["selected"])
            selections[method_name] = selected_pipeline

        prevalence = train_labels.mean(axis=0)
        majority = (prevalence >= 0.5).astype(np.int8)
        if not majority.any():
            majority[np.argmax(prevalence)] = 1
        prior_count = max(1, int(np.rint(train_labels.sum(axis=1).mean())))
        prior_top = np.zeros(train_labels.shape[1], dtype=np.int8)
        prior_top[np.argsort(-prevalence, kind="stable")[:prior_count]] = 1
        predictions["TrainingPrior"] = np.tile(majority, (len(test_ids), 1))
        predictions["TrainingPriorCardinality"] = np.tile(prior_top, (len(test_ids), 1))
        ...
        paired_final = {
            comparator: paired_node_bootstrap(test_labels, predictions["TriComponentDWASim"],
                predictions[comparator], args.bootstrap_iterations, args.seed + 12000 + offset)
            for offset, comparator in enumerate(("Cosine", "OneComponentSelector", "MagnitudeOnly", "FixedEqualMixture"))
        }
    ...
    result = {
        ...
        "paired_final_method": paired_final,
        "uncertainty_note": (
    ...
    np.savez_compressed(args.output.with_suffix(".predictions.npz"), test_ids=test_ids,
                        truth=test_labels, **predictions)
