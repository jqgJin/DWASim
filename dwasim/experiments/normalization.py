...
from dwasim.similarity import cache_identity, use_cache, safe_entropy_weights
...
def load_discrepancies(dataset: str, path_name: str, chunk_size: int) -> dict[str, np.ndarray | float]:
    ...
    identity = cache_identity(PROCESSED_ROOT, dataset, path_name)
    cache_path = CACHE_ROOT / f"heldout_discrepancies_{dataset}_{path_name}_{identity}.npz"
...
def entropy_weights(affinities: list[np.ndarray]) -> np.ndarray:
    """Use uniform weights when no path has positive information concentration."""
    return safe_entropy_weights(affinities)
