"""Historical two-component training-only optimization diagnostic.

This runner deliberately does not load the official test labels.  It compares
the historical two-path, uncalibrated majority-vote pipeline with an
...
from dwasim.paths import CACHE_ROOT, RESULTS_ROOT
from dwasim.data import PATHS, full_profile, multiply_chain
from dwasim.experiments.single_path import PAPER_PATHS
from dwasim.evaluation import deterministic_topk, majority_vote, metric_record
...
def load_training_discrepancies(dataset: str, path_name: str) -> dict:
    ...
    cache_path = CACHE_ROOT / f"nested_train_discrepancies_{dataset}_{path_name}_{identity}.npz"
...
def load_training_baseline_affinities(dataset: str, path_name: str) -> dict:
    ...
    cache_path = CACHE_ROOT / (
        f"nested_train_baselines_{BASELINE_CACHE_TAG}_{dataset}_{path_name}_{identity}.npz"
    )
    if use_cache(cache_path):
        cached = np.load(cache_path)
        return {key: cached[key] for key in cached.files}
...
def load_relative_components(dataset: str, path_name: str, discrepancy: dict) -> dict:
    ...
    train_ids, _, _, _ = load_split(dataset)
    identity = cache_identity(PROCESSED_ROOT, dataset, path_name, train_ids)
    cache_path = CACHE_ROOT / f"nested_relative_components_{dataset}_{path_name}_{identity}.npz"
    if use_cache(cache_path):
        cached = np.load(cache_path)
        return {key: cached[key] for key in cached.files}
...
def exact_affinity_from_cache(dataset: str, path_name: str, alpha: float) -> np.ndarray:
    """Reconstruct the historical global affinity via the current cache loader."""
    from dwasim.experiments.normalization import load_discrepancies
    cached = load_discrepancies(dataset, path_name, 128)
    b0 = float(cached["b0"])
    b1 = float(cached["b1"])
    distance = (
